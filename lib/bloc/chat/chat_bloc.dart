...
class ChatBloc extends Bloc<ChatEvent, ChatState> {
  ...
  ChatBloc() : super(ChatInitial()) {
    on<LoadChatEvent>((event, emit) async {
      await emit.forEach(FirebaseFirestore.instance
      ...
      .withConverter(fromFirestore: (snapshot, options) => Message.fromMap(snapshot.id, snapshot.data() ?? {}), toFirestore: (value, options) => value.toMap(),)
        ...
        return ChatLoaded(event.user.uid, data.docs.map((e) => e.data()).toList());
      });
    });

    on<SendChatMessage>((event, emit) async {
      FirebaseFirestore.instance.collection(_collectionMessages).doc().set(event.message.toMap());
    });
  }
}
